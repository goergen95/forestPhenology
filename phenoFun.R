...
calcPheno = function(index,cores){
  ...
  MIN[] = as.numeric(parallel::parApply(cl, dataArray, MARGIN=c(1,2), FUN=function(x) min(x,na.rm=TRUE)))
  AMP = MAX - MIN
  SUM[] = as.numeric(parallel::parApply(cl, dataArray, MARGIN=c(1,2), FUN=function(x) sum(x,na.rm=TRUE)))
  SD[] = as.numeric(parallel::parApply(cl, dataArray, MARGIN=c(1,2), FUN=function(x) sd(x,na.rm=TRUE)))
  Q25[] = as.numeric(parallel::parApply(cl, dataArray, MARGIN=c(1,2), FUN=function(x) quantile(x,probs=c(.25),type=7,na.rm=TRUE)))
  Q75[] = as.numeric(parallel::parApply(cl, dataArray, MARGIN=c(1,2), FUN=function(x) quantile(x,probs=c(.75),type=7,na.rm=TRUE)))
