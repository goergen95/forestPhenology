...
for (i in seq(length(res))){
  ...
  traindat = traindat[, c(1:3, 64:112)]
  ...
  mod1_season = CAST::ffs(predictors = pred, response = resp, method = "rf", importance = TRUE, trControl = trainctl, 
                   metric = "Kappa")
...
mod1_season_res25 = readRDS("data/results/mod1_season_res25.rds")

test = predict(mod1_season_res25, traindat[index,])
...
areapred = raster::predict(sta, mod1_season_res25)
writeRaster(areapred, "data/results/mod1_season_res25_areapred.tif", overwrite = TRUE)
