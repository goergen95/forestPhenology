...
tree species [@Berra2016; @Klosterman2017; @Klosterman2018; @Natesan2019; @Fricker2019; @Berra2019].
...
[@Nevalainen2017; @Yan2018a; @Onishi2018a; @Krause2019a; @Brieger2019a; @Sothe2019a]. 
...
2019 from the Marburg OpenForest research project. This area is used as a joint
research area from different scientists of several German universities who set out
to investigate questions of biodiversity and natural resource management in temperate forests. 
We artificially decrease the spatial resoultion of the imagery resulting in three different target
resolutions of 10, 15, and 25 cm. For every single overflight we calculated a series
of RGB indices on a pixels basis. Additionally, we calculate descriptive statistics
(mean, maximum, minimum, amplitude, etc.) for each index over the course of the year
to include information about the seasonality of the phenological development.

The resulting input data is used to establish 9 distinct random forest models, one
for each resolution including either only mono-temporal predictor variables, only seasonal
variables or both types of variables. On the basis of a forward-feature selection
the variables who bare the most relevant information content for the tree species
...
```{r validation_plots, echo=F, warning=F, message=F, fig.cap="Overall Accuracy of species classification on an object basis."}
