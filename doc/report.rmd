...
varImpInd$variable = reorder(varImpInd$variable, varImpInd$Kappa)

varImpSes = filter(varImTb, type == "season")
varImpSes$variable = droplevels(varImpSes$variable)
varImpSes$variable = reorder(varImpSes$variable, varImpSes$Kappa)

varImpAll = filter(varImTb, type == "all")
varImpAll$variable = droplevels(varImpAll$variable)
varImpAll$variable = reorder(varImpAll$variable, varImpAll$Kappa)

ggplot(varImpInd, aes(y=Kappa, x = reorder(variable, Kappa))) +
  geom_point(aes(shape=resolution, col=resolution)) +
  geom_segment(aes(x=reorder(variable, Kappa), xend=variable, y=0, yend=Kappa, col = resolution))+
  coord_flip() +
...
ggplot(varImpSes, aes(y=Kappa, x = reorder(variable, Kappa))) +
  geom_point(aes(shape=resolution, col=resolution)) +
  geom_segment(aes(x=reorder(variable, Kappa), xend=variable, y=0, yend=Kappa, col = resolution))+
  coord_flip() +
  facet_wrap(. ~ resolution) +
...
ggplot(varImpAll, aes(y=Kappa, x = reorder(variable, Kappa))) +
  geom_point(aes(shape=resolution, col=resolution)) +
  geom_segment(aes(x=reorder(variable, Kappa), xend=variable, y=0, yend=Kappa, col = resolution))+
  coord_flip() +
  facet_wrap(. ~ resolution) +
